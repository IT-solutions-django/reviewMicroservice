...
def gis_reviews_api(request):
    ...
    if company:
        reviews = GisReview.objects.filter(company=company, rating__gte=min_rating)
...
@require_GET
def gis_company_api(request):
    company = request.GET.get('company', '')

    if company:
        company_data = GisCompanyData.objects.filter(company=company).first()
